...
function showSlides(n) {
    ...
    var dates = document.getElementsByClassName("date"); //Récupération de la class dot
    if (n > slides.length) {
        slideIndex = 1
    }
    ...
    for (i = 0; i < dates.length; i++) {
        dates[i].className = dates[i].className.replace(" active", "");
    }

    slides[slideIndex - 1].style.display = "block"; //Modification de règle css
    dates[slideIndex - 1].className += " active";
